...
var Demo = /** @class */ (function () {
     ...
     */
    Demo.prototype.setDistanceBaby = function (distance) {
        this.performanceMonitor.newCsafeBuffer()
            ...
            }).setDistance({value: distance, unit: 36}).setProgram({value: 0})
            .addRawCommand({
                waitForResponse: false,
                command: 133,
            })
    ...
    Demo.prototype.updateScoreboard = function () {


        const player = this._players[this.currentPlayer];
        ...
        this._scoreBoard = this._scoreBoard.sort((a,b) => {
            if(typeof a.time === 'number' && typeof b.time == 'number') {
                console.warn(a.time, b.time);
                return a.time - b.time;
            }
            console.warn('not numbers');
            return 0;
        })
        ...
        for (let i = 0; i < scoreboard.length; i++) {
            ...
            let thistime = Math.floor(scoreboard[i].time / 100) /10;
    ...
    Demo.prototype.showData = function (data, general = false) {
        if(general) {
            let distance = data['workoutDuration'] - data['distance'];
            let time = Math.floor(data['elapsedTime'] / 100) /10;
            //$("#data").style($("data").style() + "font-size: 50px;");
    ...
    Demo.prototype.setCurrentPlayerDisplay = function () {
        const currPlayer =  this._players[this.currentPlayer];

        $("#currentPlayer").text("It's " + currPlayer + "'s turn!");

    }

    Demo.prototype.getPlayersFromTextArea = function () {
    ...
    Demo.prototype.changePollingRateToFastest = function () {
    ...
    Demo.prototype.getStatus = function () {
        console.error('Trying get Status...');
        this.performanceMonitor.newCsafeBuffer().addRawCommand(
            {
        ...
        ).send().then(() => console.error('Success!')).catch((e)=> console.error(e));
            ...
            /*.addRawCommand({
            waitForResponse: true,
            command: 128,//csafe.defs.SHORT_CTRL_CMDS.GETSTATUS_CMD,

            onDataReceived: function (data) {
                console.error(data.getUint8(0));
            },
    ...
    Demo.prototype.NextPlayer = function () {
        this.currentPlayer++;
    ...
    Demo.prototype.resetPM = function () {
        let newBuffer = this.performanceMonitor.newCsafeBuffer();
        newBuffer.addRawCommand({
            command: 118,
            detailCommand: 19,
            data: [1, 2],
            waitForResponse: false
        }).send().then(() => this.setDistanceBaby(100)); //F1 76 04 13 02 01 02 62 F2

            /*.addRawCommand({
                waitForResponse:false,
                command : 134//csafe.SHORT_CTRL_CMDS.GOREADY_CMD,
                //detailCommand: csafe.defs.PM_SHORT_PULL_DATA_CMDS.PM_GET_STROKESTATE,
            })
            .send()
            .then(() => {
                console.error('Great success 1');
                this.performanceMonitor.newCsafeBuffer()
                    .addRawCommand({
                        waitForResponse:false,
                        command : 135//csafe.SHORT_CTRL_CMDS.GOREADY_CMD,
                        //detailCommand: csafe.defs.PM_SHORT_PULL_DATA_CMDS.PM_GET_STROKESTATE,
                    })
                    .send().then(() => { console.error('Great succes 2');
                    this.performanceMonitor.newCsafeBuffer().addRawCommand({
                        waitForResponse:false,
                        command : 129//csafe.SHORT_CTRL_CMDS.RESET_CMD,//129

                        //detailCommand: csafe.defs.PM_SHORT_PULL_DATA_CMDS.PM_GET_STROKESTATE,
                    }).send().then(() => console.error('great succes 3!'));
                    });
            })*/
    }
    ...
    Demo.prototype.onRowingGeneralStatus = function (data) {
        this.showData(data, true); // + JSON.stringify
    ...
    Demo.prototype.onConnectionStateChanged = function (oldState, newState) {
        ...
        if (newState === ergometer.MonitorConnectionState.readyForCommunication) {
            ...
            $("#Polling").click(()=>{
                this.changePollingRateToFastest();
            })
            ...
            this.performanceMonitor.newCsafeBuffer()
                ...
                //.setWorkoutType({value: 2})

                //.setDistance({unit: 34, value: 9})
                .send(
